...
#Retrieve arguments
if len(sys.argv) != 7 or (sys.argv[1] != '-n' and sys.argv[1] != '-d' and sys.argv[1] != '-ns' and sys.argv[1] != '-ds'):
	print ("Real time audit log")
	...
	print ("Static audit log:")
	print ("  Neo4j DB Output Usage: %s -ns <Stage Directory> <Working Directory> <Program Name> <SPADE Directory> <suffix>" % sys.argv[0])
	print ("  Graphviz DOT Output Usage: %s -ds <Stage Directory> <Working Directory> <Program Name> <SPADE Directory> <suffix>" % sys.argv[0])
	quit()

stagePath = os.path.abspath(sys.argv[2])
workingPath = os.path.abspath(sys.argv[3])
isNeo4j = (sys.argv[1] == '-n' or sys.argv[1] == '-ns')
isStatic = (sys.argv[1] == '-ns' or sys.argv[1] == '-ds')
progName = sys.argv[4]
spadePath = os.path.abspath(sys.argv[5])
suffix = sys.argv[6]

if isStatic:
	startSpade(workingPath, suffix, 2, ' ')
else:
	...
	if not isNeo4j:
		...
		while not os.path.exists(outFile) or os.path.getsize(outFile) <= 162:
			...
			startSpade(workingPath, suffix, loopCount, fingerprint)
	else:
		startSpade(workingPath, suffix, 2, fingerprint)
